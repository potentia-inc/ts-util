...
export function toNumber(x) {
    ...
    if (typeof x === 'number')
        return x;
    if (typeof x === 'bigint' || typeof x === 'boolean')
        return Number(x);
    if (typeof x === 'string')
...
function decimalToBigInt(s) {
    const m = /^([+-]?)(\d+)(?:\.(\d+))?(?:[eE]([+-]?\d+))?$/.exec(s);
    if (m === null)
        throw new SyntaxError(`cannot convert to a bigint: ${s}`);
    const sign = m[1] === '-' ? '-' : '';
    const digits = m[2] + (m[3] ?? ''); // integer + fraction digits, no point
    // value = digits * 10**scale
    const scale = (m[4] !== undefined ? parseInt(m[4], 10) : 0) - (m[3]?.length ?? 0);
    if (scale >= 0)
        return BigInt(sign + digits + '0'.repeat(scale));
...
function stringToNumber(s) {
    ...
    if (isNaN(n))
        throw new TypeError(`cannot convert to a number: ${s}`);
    return n;
}
function typeName(x) {
    if (x === null)
